...
private:
    ...
    inline Color(const Color& prim, const Color& item, const Color& border, const Color* secondary, bool isBorderSet = false): 
                r_prim(prim.r_prim), g_prim(prim.g_prim), b_prim(prim.b_prim), 
                r_item(item.r_item), g_item(item.g_item), b_item(item.b_item), 
                r_border(border.r_border), g_border(border.g_border), b_border(border.b_border),
...
public:
    ...
    inline Color operator|(const Color& other)     const {
        LOGGER_PATTERN("isBorderSet: _, prim: (_, _, _), item: (_, _, _), border: (_, _, _)", isBorderSet, r_prim, g_prim, b_prim, r_item, g_item, b_item, r_border, g_border, b_border)
        if (isBorderSet) return Color(*this, other, getBorderColor(), secondaryColor);
        else return Color(*this, getItemColor(), other, secondaryColor, true);
    }
